...
Zwei separate Netzwerke mit einem Vermittlungsrechner verbinden, sodass PCs aus Netz A mit PCs aus Netz B kommunizieren können.
...
1. Im **Entwurfsmodus**: Platziere:
   ...
   - 1 Vermittlungsrechner (in der Mitte)
   - 2 PCs links (PC-A1, PC-A2), verbunden mit Switch-A
   - 2 PCs rechts (PC-B1, PC-B2), verbunden mit Switch-B

2. Verbinde Switch-A mit dem Vermittlungsrechner (Kabel)
3. Verbinde Switch-B mit dem Vermittlungsrechner (Kabel)

## Schritt 2: Vermittlungsrechner konfigurieren

1. **Doppelklicke** auf den Vermittlungsrechner
...
> **Wichtig:** Der Gateway ist die IP-Adresse des Vermittlungsrechners im eigenen Netzwerk!
...
3. Teste:
   ...
   - `ping 192.168.2.1` → PC-B1 im anderen Netz ✅ (über Vermittlungsrechner)
   - `ping 192.168.2.2` → PC-B2 im anderen Netz ✅ (über Vermittlungsrechner)
...
| Vermittlungsrechner antwortet nicht | Schnittstellen des Vermittlungsrechners nicht richtig konfiguriert |
